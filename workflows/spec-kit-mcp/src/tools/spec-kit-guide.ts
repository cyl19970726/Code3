...
CRITICAL: You MUST use spec-kit MCP Prompts (specify, clarify, plan, tasks, analyze, implement) at each stage.
...
export async function handleSpecKitGuide(): Promise<any> {
  return {
    ...
    nextSteps: [
      '0. Call spec-kit-mcp init tool (projectPath) to create .specify/ structure',
      '   → Verify: ls -R .specify/ shows 4 scripts, 3 templates, 1 constitution',
      '',
      '1. Call spec-kit-mcp specify Prompt (featureDescription) → generates spec.md',
      '   → LLM receives instructions → executes → writes spec.md',
      '',
      '2. Call spec-kit-mcp spec-context tool (specPath) → verify spec.md',
      '   → Check: character count >= 8k, requirements: 12-20',
      '',
      '3. Call spec-kit-mcp clarify Prompt (specPath) → ask 5 questions, update spec.md',
      '   → LLM scans 11 ambiguity types → asks user interactively → updates spec.md',
      '',
      '4. Call spec-kit-mcp spec-context tool (specPath) → verify Clarifications section exists',
      '',
      '5. Call spec-kit-mcp plan Prompt (specPath) → generates plan.md',
      '   → LLM reads spec.md, constitution.md, plan-template.md → generates plan.md',
      '',
      '6. Call spec-kit-mcp plan-context tool (planPath) → verify plan.md',
      '   → Check: tech stack, data model, 5 phases',
      '',
      '7. Call spec-kit-mcp tasks Prompt (planPath) → generates tasks.md',
      '   → LLM reads plan.md, tasks-template.md → generates tasks.md (20+ tasks)',
      '',
      '8. Call spec-kit-mcp tasks-context tool (tasksPath) → verify tasks.md',
      '   → Check: total tasks >= 20, 5 phases',
      '',
      '9. Call spec-kit-mcp analyze Prompt (specPath, planPath, tasksPath) → quality report',
      '   → LLM performs 6 quality checks → generates report → asks user to apply fixes',
      '',
      '10. Optional: Call spec-kit-mcp implement Prompt (tasksPath) → TDD implementation',
